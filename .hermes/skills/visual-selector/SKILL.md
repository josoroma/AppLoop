...
| `generated-nextjs-ai-engineer-cv` | `template-ai-engineer-cv` |
| `generated-nextjs-deep-research-paper` | `template-deep-research-paper` |
| `generated-nextjs-webgl-particles-home` | `template-webgl-particles-home` |
...
- **Templates share inspector-provider but differ at root**: The templates share a common `inspector-provider.tsx` pattern (keep selector behavior in sync). However, layouts and shell components differ, and each template has its own body classname (`template-default`, `template-admin-luma`, `template-ai-engineer-cv`, `template-deep-research-paper`, or `template-webgl-particles-home`). When adding a new inspector event listener, apply it to every template's provider and verify all insertion points landed: (1) handler function definition, (2) `addEventListener` registration, (3) `removeEventListener` cleanup. Do NOT blindly copy layout or shell files across templates — sync only the provider behavior that should truly be shared.
