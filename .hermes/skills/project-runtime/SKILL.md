...
### `templates/*/.next` Lint Noise

Template directories accumulate `.next` build output when they're used as edit/preview workspaces. These generated directories contain thousands of JS files from Turbopack/Three.js that will be picked up by the AppLoop builder's `npm run lint`, producing 10,000+ warnings/errors from `node_modules/` style code, `require()` imports, and ESLint-ignored turbopack-runtime patterns.

**Fix**: Clear temporary build output before linting the builder:

```bash
rm -rf templates/*/.next && npm run lint
```

This is safe because `.next` is always regenerated by Next.js on the next dev-server start. Template source files under `templates/` are not affected.
