...
- Accept and ignore known legacy/template-local tokens AppLoop does not store as theme semantics. Baseline `IGNORED_CUSTOM_THEME_TOKENS` currently includes at least `--destructive-foreground` and `--board`; do not fail the whole theme for those.
- When the AI generates another template-specific CSS variable (e.g. more chassis vars, component gaps), add it to `IGNORED_CUSTOM_THEME_TOKENS` in `lib/themes/registry.ts`. These belong in the ignore set — not `REQUIRED_THEME_TOKENS` (which would force every theme to define them). `parseTokenBlock()` rejects any token not in either set; ignored vars are dropped from serialized custom theme maps, while durable styling stays in the template `globals.css`.
- Symptom of a missing ignored token: “Create Template” fails immediately at `createCustomTheme(themeCss)` with `Custom themes cannot define unsupported token --board` (or the generated var name). Fix by ignoring the var — do not promote layout vars into `REQUIRED_THEME_TOKENS`, and do not weaken validation for imports/URLs/arbitrary selectors.
